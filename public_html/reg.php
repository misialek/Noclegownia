    ...
    if ($_POST["wyslane"]) { 
       ...
       if ((strlen($login) < 3 or strlen($login) > 30) and (preg_match('/^[a-z\d_]{2,20}$/i', $login))){
        ...
        if ($blad == 0) {
            ...
            $wynik = mysql_query("INSERT INTO $tabela VALUES('','30', '$imie', '$nazwisko', '$login', '$haslo', '$email', '$kod', NOW(),'', '')");
            if ($wynik) {
               $list = "Witaj $login!
Kliknij w poniższy link, aby aktywować swoje konto. http://noclegownia.keed.pl/weryfikacja.php?weryfikacja=potwierdz&kod=$kod";
                mail($email, "Rejestracja uzytkownika", $list, $mail_to);
