  ...
  <body>
    ...
    <h2><?php echo $noclegownia->__get('nazwa') . " - ul. " . $noclegownia->__get('ulica') . ", " . $noclegownia->__get('kod_pocztowy') . " " . $noclegownia->__get('miejscowosc'); ?></h2>
     ...
     <?php if(!isset($dostepnePokoje[$noclegowniaId])): ?>
      <span style="font-size:12px;">Wybrane miejsce nie ma dostepnych pokoi. </span><input type="button" value="Dodaj pokój" onclick="location.href = 'index.php?akcja=dodajPokoj&id=<?php echo $noclegowniaId; ?>';" /><br /><br />
     <?php else: ?>
      <table align="center">
        <tr>
          <th>
        ...
        <?php foreach($dostepnePokoje[$noclegowniaId] as $pokoj): ?>
         <tr>
            ...
            <td>
            <input type="button" value="Podglad pokoju" onclick="location.href = 'index.php?akcja=listujPokoje&pokoj=<?php echo $pokoj->__get('id_pok')?><?php if(isset($_GET['id'])){echo '&id='.$_GET['id'].'';} ?>';" />
            </td>
          </tr>
        <?php endforeach;?>
      </table>
