...
if((strlen($typ_miejsca) != 0) AND (strlen($miasto) == 0) AND (strlen($pobyt_od) != 0) AND (strlen($pobyt_do) != 0) AND ($tv == '0') AND ($lodowka == '0') AND ($wc == '0') AND ($prysznic == '0') AND ($wanna == '0') AND ($jacuzzi == '0') AND ($klimatyzacja == '0') AND ($internet == '0')){
	...
	FROM pokoj
	JOIN noclegownia ON (id_miejsce = id)
	WHERE id_pok NOT IN (SELECT id_pokoj FROM rezerwacje WHERE status = 1 AND
	((data_od <= '$pobyt_od'
	...
	$id_noclegownia=$result['id'];
			echo '<article class="col2">
                ...
                echo '</div>
						...
						FROM pokoj
						WHERE id_miejsce = $id_noclegownia AND id_pok NOT IN (SELECT id_pokoj FROM rezerwacje WHERE status = 1 AND
						((data_od <= '$pobyt_od'
						AND data_do <= '$pobyt_do'
						AND data_do >= '$pobyt_od') OR
						(data_od >= '$pobyt_od' 
...
if((strlen($typ_miejsca) != 0) AND (strlen($miasto) != 0) AND (strlen($pobyt_od) != 0) AND (strlen($pobyt_do) != 0) AND ($tv == '0') AND ($lodowka == '0') AND ($wc == '0') AND ($prysznic == '0') AND ($wanna == '0') AND ($jacuzzi == '0') AND ($klimatyzacja == '0') AND ($internet == '0')){
	...
	FROM pokoj
	JOIN noclegownia ON (id_miejsce = id)
	WHERE typ='$typ_miejsca' AND miejscowosc='$miasto' AND id_pok NOT IN (SELECT id_pokoj FROM rezerwacje WHERE status = 1 AND
	((data_od <= '$pobyt_od'
	AND data_do <= '$pobyt_do'
	AND data_do >= '$pobyt_od') OR
	(data_od >= '$pobyt_od' 
	...
	$id_noclegownia=$result['id'];
			echo '<article class="col2">
                ...
                echo '</div>
						...
						id_pok NOT IN (SELECT id_pokoj FROM rezerwacje WHERE status = 1 AND
						((data_od <= '$pobyt_od'
						AND data_do <= '$pobyt_do'
						AND data_do >= '$pobyt_od') OR
						(data_od >= '$pobyt_od' 
