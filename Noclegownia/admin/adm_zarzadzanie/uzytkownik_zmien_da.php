...
echo '<center><b><h1>Edycja danych osobowych</h1></b><br /><br /><br />
<table width="100%" align="center"><form action="uzytkownik_zmien_dane.php" method="post">
	<input type="hidden" name="zmien_dane" value="TRUE" />
		<tr class="reglbl"><td><strong>Imie:</strong></td><td><input type="text" name="imie" value="'.$pole['imie'].'" /></td></tr>
		<tr class="reglbl"><td><strong>Nazwisko:</strong></td><td><input type="text" name="nazwisko" value="'.$pole['nazwisko'].'" /></td></tr>
		<tr class="reglbl"><td><strong>E-mail:</strong></td><td><input type="text" name="email" value="'.$pole['email'].'" /></td></tr>';
		...
		<tr><td><input type="radio" name="typ" value="30" '; if($pole['typ_konta']==30){echo 'checked';} echo'/>Użytkownik</td>
			...
			<tr><td><strong>Zmień noclegownie:</strong></td><td>
				<select '; if($pole['typ_konta']!=20){echo 'DISABLED';} echo ' name="nazwa_nocleg">
					...
					echo '</optgroup>';
				echo'</select><br />
		<tr class="reglbl"><td><strong>Nazwa obsługiwanej noclegowni:<Strong></td><td>'.$moja['nazwa'].'</td></tr>
		<tr class="reglbl"><td><input type="hidden" name="id_u" value="'.$id.'" /></td></tr></table>
		<input type="submit" value="Zmień" /></form></center>
    </form>
</center>';
