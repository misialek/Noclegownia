...
Nowe hasło to: $haslo_mail";
			$mail=mysql_query("SELECT * FROM $tabela WHERE login='$login'");
			$email=mysql_fetch_assoc($mail);
            mail($email['email'], "Zmiana hasła", $list, $mail_to);
			echo '<a href="uzytkownik.php"><button>Wstecz</button></a>&nbsp;&nbsp;
			Hasło zostało zmienione.<br />&nbsp;Zaloguj się ponownie.';}
        }else{echo '<a href="uzytkownik_has.php"><button>Wstecz</button></a>';}
        }
